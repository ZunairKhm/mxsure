...
#'
#' @export
mxsure_estimate_tree <- function(mixed_snp_dist, unrelated_snp_dist, mixed_time_dist=NA, mixed_sites=NA,truncation_point=2000,
                               youden=FALSE,threshold_range=FALSE, max_time= NA, start_params= NA,
                            tree=NA, sampleA=NA, sampleB=NA, return_corrected=FALSE,
  ...
  compute_corrected_time <- function(tip1, tip2, snp_dist, time_diff) {
    for (tree_i in tree_list) {
      ...
      if (tip1 %in% tips && tip2 %in% tips) {
        ...
        if (is.na(distance_since_sample) || distance_since_sample == 0) return(time_diff)
