...
#' Mixture SNP Cutoff (mcclust)
#'
#' Same as mixture_snp_cutoff but uses a gaussian mixture model for the unrelated SNP distances.
#'
...
#'
#' @export
mixture_snp_cutoff_mcclust <- function(trans_snp_dist, unrelated_snp_dist, trans_time_dist=NA, trans_sites=NA,
  ...
  if(!anyNA(trans_time_dist)&(!anyNA(trans_sites))){
    if ((length(trans_snp_dist) >= 30) && (length(unrelated_snp_dist) >= 30)){
      nb_fit <- Mclust(unrelated_snp_dist)
      ...
      llk <- function(params, x, t, s){
        ...
        -sum(pmap_dbl(list(x, t,s), ~ {log_sum_exp(log(k) + dpois(x = ..1,
                                                                  ...
                                                                  log = TRUE),
                                                   log(1-k) + dens(..1, nb_fit$modelName, mc.fit$parameters))}))
      }
