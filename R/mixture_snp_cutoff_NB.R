...
#' Mixture SNP Cutoff (Negative Binomial)
#'
#' Same as mixture_snp_cutoff but uses a negative binomial distribution to model the related SNP distances.
#'
...
#'
#' @export
mixture_snp_cutoff_NB <- function(trans_snp_dist, unrelated_snp_dist, trans_time_dist=NA, trans_sites=NA,
  ...
  if(!anyNA(trans_time_dist)&(!anyNA(trans_sites))){
    if ((length(trans_snp_dist) >= 30) && (length(unrelated_snp_dist) >= 30)){
      ...
      llk <- function(params, x, t, s){
        ...
        -sum(pmap_dbl(list(x, t,s), ~ {log_sum_exp(log(k) + dnbinom(x = ..1,
                                                                  mu =  lambda*..2*(..3/1000000), #gives rate esimate per day time per million bp
                                                                  size= nbsize,
                                                                  log = TRUE),
                                                   log(1-k) + dnbinom(x = ..1,
                                                                      size = nb_fit$estimate['size'],
        ...
        result = nlminb(start=c(result$par[[1]], result$par[[2]], 0), objective=llk, x = trans_snp_dist, t = trans_time_dist, s = trans_sites,
                         lower = c(0, 1e-10, 0), upper = c(1, Inf, Inf), control = list(trace = trace))
