...
#' Mixture SNP Cutoff (Logspline)
#'
#' Same as mixture_snp_cutoff but uses a logspline distribution to model the unrelated SNP distances.
#'
...
#'
#' @export
mixture_snp_cutoff_logspline <- function(trans_snp_dist, unrelated_snp_dist, trans_time_dist=NA, trans_sites=NA,
  ...
  if(!anyNA(trans_time_dist)&(!anyNA(trans_sites))){
    if ((length(trans_snp_dist) >= 30) && (length(unrelated_snp_dist) >= 30)){
      nb_fit <- logspline(unrelated_snp_dist, lbound = 0)
